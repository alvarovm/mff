...
def carve_confs(filename, r_cut):
    ...
    print('Database will have {} entries'.format(n_data))
    ...
    if len(elementslist) == 2:
        ...
        # Choose the number of entries each element will have,
        # proportional to the square root of the ratio of occurrences of each element
        ...
        for i in np.arange(nc_el1):
            ...
            cell = atoms.get_cell()
            ind_atom = int(
                i % len(elplace[0, 0]))  # Select the atom number rotating between the total atoms of el 1 present #
            ...
            if errvalue == 2:
                raise KeyError('Cannot find energy or force values in the xyz file, shutting down now')
        ...
        for i in np.arange(nc_el1):
            ...
            nl.build(atoms)
            cell = atoms.get_cell()
            ind_atom = int(i % N)  # Select the atom number rotating between the total atoms of el 1 present #
    ...
    np.save("energies__cut={:.2f}.npy".format(r_cut), energies)
    lens = []
